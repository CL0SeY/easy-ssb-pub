...
RUN apt-get install -y curl libc6 libcurl3 zlib1g libtool autoconf
...
RUN . $HOME/.nvm/nvm.sh && npx tsc -v && npx node -v && npx tsc
CMD . $HOME/.nvm/nvm.sh && npm run start --host $HOST
